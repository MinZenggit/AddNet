...
// Helper struct to hold all parameter estimates for one method (UNCHANGED)
struct ParamSet {
  arma::mat gamma_hat;
  arma::cube gamma_var;
  arma::mat alpha_hat;
...
// Helper function to process one slice (k) for one method (ENTIRELY UNCHANGED)
void process_k_slice(
    int k, int n, int p, double delta_t,
    const arma::mat& current_events,
    const arma::cube& z_k, // Renamed from z_constant for clarity
  ...
  if (p > 0) {
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < n; ++j) {
        Z_gamma_k(i, j) = dot(arma::vec(z_k.tube(i, j)), params.gamma_hat.col(k));
      }
    }
  }
  ...
  // --- Step 5: Estimate Variances (Logic is identical) ---
...
List compute_results_flexible_cpp(int n, int p, 
                                  ...
                                  const Rcpp::List& precomputed_z_list,
                                  const Rcpp::IntegerVector& z_map_indices) {
  ...
  // --- Main loop over K with Z-set lookup ---
  for (int k = 0; k < K; ++k) {
    // 1. Look up which Z-set to use for this time slice k
    int z_idx = z_map_indices[k] - 1; // R to C++ index
    Rcpp::List current_z_set = precomputed_z_list[z_idx];
    
    // 2. Extract the pre-computed matrices for this set
    const arma::cube& z_k = Rcpp::as<arma::cube>(current_z_set["z_k"]);
    const arma::mat& inv_XtX = Rcpp::as<arma::mat>(current_z_set["inv_XtX"]);
    const arma::mat& diff2_z_core = Rcpp::as<arma::mat>(current_z_set["diff2_z_core"]);
    const arma::cube& diff2_z_cube = Rcpp::as<arma::cube>(current_z_set["diff2_z_cube"]);
    const arma::cube& z_diff_alpha = Rcpp::as<arma::cube>(current_z_set["z_diff_alpha"]);
    const arma::cube& z_diff_beta = Rcpp::as<arma::cube>(current_z_set["z_diff_beta"]);
    
    // 3. Call the UNCHANGED worker function for the truncate method
    process_k_slice(k, n, p, delta_t_truncate, event_counts_truncate.slice(k),
                    z_k, diff2_z_core, inv_XtX, diff2_z_cube,
                    z_diff_alpha, z_diff_beta, params_truncate);
    
    // 4. Call the UNCHANGED worker function for the kernel method (if needed)
    if (compute_kernel) {
  ...
  // Return results (logic is UNCHANGED)
  if (compute_kernel) {
    return List::create(
